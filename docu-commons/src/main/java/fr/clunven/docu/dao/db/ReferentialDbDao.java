...
public class ReferentialDbDao extends AbstractDaoSupport {
    ...
    private List < GenreDto > getListGenreTopLevel() {
    ...
    private List < SubGenreDto > getListGenreSubLevel(int parent) {
    ...
    public Map < GenreDto, List < SubGenreDto > > getMenu() {
        ...
        getListGenreTopLevel().forEach(g -> menu.put(g, getListGenreSubLevel(g.getId())));
        return menu;
    }
    ...
    public List < String > getListOfChildGenre(int parent) {
        if (!getChildGenres().containsKey(parent)) {
            ...
            getChildGenres().put(parent,
                    getJdbcTemplate().query("SELECT NOM from ref_genre WHERE PARENT = ?" , new SingleColumnRowMapper<>(), parent));
        }
